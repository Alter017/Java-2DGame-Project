...
//	int messageCounter = 0;
	...
	public void drawMessage() {
		...
		if (!message.isEmpty()) {
		    // Always take the *last* message (the newest one)
		    String latestMessage = message.get(message.size() - 1);

		    g2.setColor(Color.black);
		    g2.drawString(latestMessage, messageX + 2, messageY + 2);
		    g2.setColor(Color.white);
		    g2.drawString(latestMessage, messageX, messageY);

		    // Update counter for the latest message only
		    int lastIndex = message.size() - 1;
		    int counter = messageCounter.get(lastIndex) + 1;
		    messageCounter.set(lastIndex, counter);

		    // Remove when expired
		    if (counter > 180) {
		        message.remove(lastIndex);
		        messageCounter.remove(lastIndex);
		    }

		    // Optional: clear all older messages immediately so only the latest shows
		    while (message.size() > 1) {
		        message.remove(0);
		        messageCounter.remove(0);
		    }
		}
	...
	public void drawTitleScreen() {
		...
		if(titleScreenState == 0) {
			Color green = new Color(70,120,80);
